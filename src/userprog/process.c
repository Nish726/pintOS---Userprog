...
process_execute (const char *file_name) 
{
  ...
  /* Allocates dynamic memory to store a copy of the file_name string. 
     After the code execution, _file_name would point to the first token
     in the original file_name string, and save_pointer would contain 
     the information to continue parsing the string from the next token.*/
  _file_name = malloc(strlen(file_name) + 1);
  strlcpy (_file_name, file_name, strlen(file_name)+1);
  _file_name = strtok_r((char*) _file_name, " ", &save_pointer );
  ...
  /* Should free _file_name at the end. */
  free(_file_name);
  return tid;
}
...
start_process (void *file_name_)
{
  ...
  /* his code sets a custom status field in the current thread and
     then signals a semaphore, possibly indicating the successful completion 
     of a specific operation related to thread initialization. */
...
int process_wait(tid_t child_tid) {
    ...
    // Search for the child with the specified tid
    struct list_elem *child_elem;
    for (child_elem = list_begin(&curr_t->child_list);
         child_elem != list_end(&curr_t->child_list);
         child_elem = list_next(child_elem)) {
        struct thread *t = list_entry(child_elem, struct thread, child_elem);
        if (t->tid == child_tid) {
            child_t = t;
            break;
        }
  ...
  if (kpage != NULL) 
  {
    ...
    if (success) {
      *esp = PHYS_BASE;
      ...
      _file_name_ = palloc_get_page(0);
      if (_file_name_ == NULL)
        return TID_ERROR;
      strlcpy(_file_name_, file_name_, PGSIZE);

      cmd_args_arr = malloc(DEFAULT_ARG_SIZE*sizeof(char*));
      for (
        token = strtok_r(_file_name_, " ", &save_ptr); 
        token != NULL;
      ...
      for (int i=arg_count; i>0; i--) {
        arg_len = strlen(cmd_args_arr[i-1]) + 1; 
        byte_size += arg_len;
        *esp -= arg_len;
